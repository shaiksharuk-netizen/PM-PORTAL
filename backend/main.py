...
# main.py - Add this endpoint after /api/upload-file (around line 1700)

# --- NEW: Google Drive Upload Endpoint ---

@app.post("/api/upload-from-drive")
async def upload_from_drive(
    request_data: DriveUploadRequest,
    background_tasks: BackgroundTasks = None,
    db: Session = Depends(get_db)
):
    """
...
async def ask_chatbot_question(
    ...
    print(f"\n{'='*80}")
    print("[ASK-QUESTION] New question received")
    ...
    try:
        ...
        if file_context:
            ...
            print("[ASK-QUESTION] Using provided file_context")

        elif file_id:
            uploaded_file = db.query(UploadedFile).filter(UploadedFile.id == file_id).first()
            if not uploaded_file:
                return {"success": False, "error": "File not found", "chat_id": chat_id}
            ...
            print("[ASK-QUESTION] No specific file provided, searching KB")
        ...
        if use_pinecone_search:
            ...
            if knowledge_base_files:
                kb_file_ids = [kb.mandatory_file_id for kb in knowledge_base_files]
                ...
                for file in all_mandatory_files:
                    idx = pinecone_service.get_index_name_for_file(file.id, file.file_name)
        ...
        # ================= ROUTER =================
        if use_router_answerer:
            router_data = _search_across_all_files_and_route(question, top_k=10, db=db)

            if not router_data["context_chunks"]:
                error_text = "No relevant documents found in the knowledge base."
                _save_chat_message(db, chat_id, "assistant", error_text, user_email)
                return {"success": False, "error": error_text, "chat_id": chat_id}
...
"""
            ...
            llm_response = re.sub(r"```.*?```", "", llm_response, flags=re.S).strip()

            _save_chat_message(db, chat_id, "assistant", llm_response, user_email)
            return {"success": True, "response": llm_response, "chat_id": chat_id}

        # ================= BIG PROMPT (UNCHANGED) =================
        elif context_text:
...
- Do NOT simply repeat the document content
- **CRITICAL: Preserve ALL external links**
- Convert links to HTML <a href="url" target="_blank">text</a>
- Always include Google Docs / Sheets links
- Do NOT skip URLs
